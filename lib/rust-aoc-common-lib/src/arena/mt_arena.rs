...
use std::{ fmt::Debug, marker::{ Send, Sync }, sync::{ Arc, RwLock }, thread::{ spawn, JoinHandle } };

use super::{ m_arena::Arena, Node, ResultUidList, ShareableArena, WalkerFn };
use crate::utility::ReadGuarded;
...
#[derive(Debug)]
pub struct MTArena<T> where T: Debug + Send + Sync + Clone + 'static {
    arena_arc: ShareableArena<T>,
}

impl<T> MTArena<T> where T: Debug + Send + Sync + Clone + 'static {
    pub fn new() -> Self {
        MTArena {
...
impl<T> Default for MTArena<T> where T: Debug + Send + Sync + Clone + 'static {
    fn default() -> Self {
        Self::new()
    }
}