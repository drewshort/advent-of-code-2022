...
/// ```
#[derive(Debug)]
pub struct LazyMemoValues<F, T, V> where F: FnMut(&T) -> V, T: Clone + Eq + Hash, V: Clone {
    pub create_value_fn: F,
    pub value_map: HashMap<T, V>,
}

impl<F, T, V> LazyMemoValues<F, T, V> where F: FnMut(&T) -> V, T: Clone + Eq + Hash, V: Clone {
    ...
    pub fn get_copy(&mut self, arg: &T) -> V {
