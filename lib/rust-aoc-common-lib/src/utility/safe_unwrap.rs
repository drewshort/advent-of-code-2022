...
use std::{ fmt::Debug, sync::{ Arc, RwLock, RwLockReadGuard, RwLockWriteGuard } };
...
pub fn with_mut<T, F, R>(arg: &mut T, receiver_fn: &mut F) -> R where F: FnMut(&mut T) -> R {
    receiver_fn(arg)
}

pub fn with<T, F, R>(arg: T, receiver_fn: F) -> R where F: Fn(T) -> R {
    receiver_fn(arg)
}

pub fn call_if_some<T, F>(option_wrapped_value: &Option<T>, receiver_fn: &F) where F: Fn(&T) {
    if let Some(value) = option_wrapped_value {
        receiver_fn(value);
    }
}

pub fn call_if_none<T, F>(option_wrapped_value: &Option<T>, receiver_fn: &F) where F: Fn() {
    if option_wrapped_value.is_none() {
        receiver_fn();
    }
}

pub fn call_if_ok<T, F, E>(option_wrapped_value: &Result<T, E>, receiver_fn: &F) where F: Fn(&T) {
