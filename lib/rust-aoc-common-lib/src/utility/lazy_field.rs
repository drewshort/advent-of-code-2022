...
pub trait LazyExecutor<T> where T: Send + Sync {
    fn compute(&mut self) -> T;

    /// Book: <https://doc.rust-lang.org/book/ch10-02-traits.html>
    #[allow(clippy::all)]
    fn new() -> Box<dyn LazyExecutor<T> + Send + Sync> where Self: Default + Sized + Sync + Send + 'static {
        Box::new(Self::default())
    }
}

pub struct LazyField<T> where T: Send + Sync {
    pub lazy_executor: Box<dyn LazyExecutor<T> + Send + Sync>,
    pub field: T,
    pub has_computed: bool,
}

impl<T> LazyField<T> where T: Send + Sync, T: Default + Clone {
